...
This GitHub action wraps the Fly.io CLI to automatically deploy pull requests to [fly.io](http://fly.io) for review. 
These are useful for testing changes on a branch without having to setup explicit preview environments.

This action will create, deploy, and destroy Fly apps. Just set an Action Secret for `FLY_API_TOKEN`.

If you have an existing `fly.toml` in your repo, this action will copy it with a new name when deploying. 
...
| name       | description                                                                                                                                                                                                          |
|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `app`      | The application name of the Fly app. Alternatively, set the env `FLY_APP`. For safety, must include the PR number. Example: `myapp-pr-${{ github.event.number }}`. Defaults to `pr-{number}-{repo_org}-{repo_name}`. |
...
jobs:
  preview_app:
    ...
    # Create a GitHub deployment environment per preview app so it shows up
...
For production apps, it's a good idea to create a new Postgres cluster specifically for preview apps.
...
steps:
  ...
  - name: Deploy app
    ...
    uses: superfly/fly-pr-review-apps@1.0.0
    with:
      postgres: myapp-postgres-preview-apps
```

## Example with multiple Fly apps

If you need to run multiple Fly apps per preview app, for example Redis, memcached, etc, just give each app a unique name. Your application code will need to be able to discover the app hostnames.
...
steps:
  ...
  - name: Deploy redis
    ...
    with:
      ...
      app: pr-${{ github.event.number }}-myapp-redis

  - name: Deploy app
    id: deploy
    uses: superfly/fly-pr-review-apps@1.0.0
    with:
      app: pr-${{ github.event.number }}-myapp-app
```
